...
public class ShipMovementWithGuided : MonoBehaviour
{
    ...
    private float guidedProjectionSpeed = 10f;
            ...
            else
            {
                ...
                if (Time.time >= nextTimeToGuidedFire && dotProd > 0.9)
                {
                    ...
                    foreach (Transform guidedProjectileOrigin in guidedProjectileOrigins)
                    {
                        if (target != null)
                        {
                            var guidedProjectile = Instantiate(guidedProjectilePrefab, guidedProjectileOrigin.position, guidedProjectileOrigin.rotation);
                            guidedProjectile.GetComponent<GuidedProjectile>().target = target;
                            guidedProjectile.GetComponent<Rigidbody>().AddForce(guidedProjectileOrigin.forward * guidedProjectionSpeed, ForceMode.VelocityChange);
                        }
                    }
                }            
            }
