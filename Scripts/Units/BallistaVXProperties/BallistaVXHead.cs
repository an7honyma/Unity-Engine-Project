...
public class BallistaVXHead : MonoBehaviour
{
    public float fireRate = 7.5f;

    private float nextTimeToFire = 0f;
    public Transform projectileOrigin;
    public GameObject ballistaVXProjectilePrefab;
    ...
    void Operate()
    {
        Transform target = tankBody.GetComponent<BallistaVXMovement>().target;
        if (target != null)
        {
            ...
            if (tankBody.GetComponent<BallistaVXMovement>().inRange)
            {
                ...
                if (Time.time >= nextTimeToFire && dotProd > 0.9)
                {
                    ...
                    var projectile = Instantiate(ballistaVXProjectilePrefab, projectileOrigin.position, projectileOrigin.rotation);
